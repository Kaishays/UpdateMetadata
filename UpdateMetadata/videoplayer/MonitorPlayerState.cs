/*using KlvExtractor.ManageExtraction;
*/using KlvPlayer;
using StCoreWr;
...
namespace KlvExtractor.VideoPlayer
{
    public static class MonitorPlayerState
    {
        ...
        {
            while (cKlvPlayer.PlayerState != Player_State.Completed)
            {/*
                await Task.Delay(RuntimeVariables.delayMonitorVideoPlayerState);*/
                if (HandleCorruptedVids.CheckPlayerStateError(cKlvPlayer.PlayerState))
                {
                    ResetPlayer.Reset();
                    return false;
                }
